...
class OrderClient(
    ...
    fun updateOrderUserAddressTo(orderId: String, userAddressTo: String): ResponseEntity<OrderDto> {
        val uri = createBuilder("api/v1/order/$orderId/address/to")

        return restTemplate.exchange(uri.toUriString(), HttpMethod.PUT, HttpEntity(UpdateUserAddressToRequest(userAddressTo = userAddressTo)), OrderDto::class.java)
    }
